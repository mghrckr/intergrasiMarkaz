...
import { fetchUsers, fetchTrxs, fetchLaba, fetchKodeOperators, fetchModuleTrxs, fetchSaldoMembers } from '../store/actionCreators';
                ...
                <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 table-fixed shadow-md sm:rounded-lg">
                    <thead className="bg-gray-50">
                        <tr>
                            ...
                            <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider" style={{ width: '500px' }}>ID</th>
                            <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider" style={{ width: '200px' }}>Nama</th>
                            <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider" style={{ width: '300px' }}>Saldo</th>
                            <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider" style={{ width: '200px' }}>Limit</th>
                        </tr>
                    </thead>
                    <tbody>
                        {saldo?.data_member_saldo_negatif?.map((s, index) => {
                                return (
                                    <tr key={index}>
                                        <td className='text-center' style={{ color: 'black' }}>{index + 1}</td>
                                        <td className='text-center' style={{ color: 'black' }}>{s.ID}</td>
                                        <td className='text-center' style={{ color: 'black' }}>{formatNumber(s.Nama)}</td>
                                        <td className='text-center' style={{ color: 'black' }}>{formatNumber(s.Saldo)}</td>
                                        <td className='text-center' style={{ color: 'black' }}>{formatNumber(s.Limit)}</td>
                                    </tr>
                                );
                            })}
                        <tr>
...
export default TableMemberSaldo;
